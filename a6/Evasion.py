...
def calculate_heat(steps,hunter_vx,hunter_vy,hunter_x,hunter_y,heatmap):   
    ...
    canvas.create_rectangle(minx, miny, maxx, maxy, fill = "yellow", tag = 'predict')
    canvas.update() 
    ...
    while (x_prey,y_prey)!=pos:
        ...
        for move in temp_moves:
            ...
            if not (mx,my) in checked:
                if not(vertical_check(mx)):
                    if not(horizontal_check(my)):
                        temp_queue.append((heatmap[mx][my]+1000*euclidean_distance((mx,my),(x_prey,y_prey)),(move[0],move[1])))
...
#    canvas.create_oval(x[t], y[t], x[t]+ball_diameter, y[t]+ball_diameter, fill="blue", tag='blueball')
    ...
    if tick % 2 and tick > 0:
        
        if len(prey_queue)==0:
        
            hunter_predict_x = h_x - last_hunter_x
            hunter_predict_y = h_y - last_hunter_y
            if(hunter_predict_x != last_hunter_x-h_x) or (hunter_predict_y != last_hunter_y-h_y):                                
                heatmap = [[100 for value in range(500)] for value in range(500)] 
