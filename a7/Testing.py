...
def greedy_neighbor(munched, nodes, edges, edges_data, otherNewMunchers):
    ...
    for enemy in consider:
        ...
        for (direction, node) in edges[enemy].iteritems():
#            if node in nbh or node == enemy:
#                continue
            if node not in munched and len(edges[enemy].keys()) == 1:
            ...
            elif node not in munched and direction_win(direction, direction_inverse(direction)):
...
def greedyMove(munched,nodes,edges, edges_data, otherNewMunchers, round_no, subGs):
    ...
    if (remainingStuff[0] > 0 and otherNewMunchers):
        scorchers = greedy_neighbor(munched, nodes, edges, edges_data, otherLiveMunchers)
