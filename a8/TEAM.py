...
#                    continue
                if node not in nbh and node not in munched and len(edges[neighbor].keys()) == 1:
...
#        pdb.set_trace()
        ...
        num_g = max(len(otherNewMunchers) - len(scorchers), len(greedys))
	G.remove_nodes_from(munched)
	all_greedys = greedy_global(munched, G.nodes(), edges, edges_data)
	greedys = greedys + all_greedys
