...
#        pdb.set_trace()
        ...
        for trash in range(size_b):
            ...
            ws[trash] = float_to_int_weights(get_weight())
...
#        pdb.set_trace()
        app = i % 2
        candidate = ""
#        npph = np.percentile(w_std, 55)
#        nppl = np.percentile(w_std, 33)
        for j in range(len(w)):
#            if True:
            if w_std[j] > w_std_mean/np.sqrt(n) or i == 19:# or w_std[j] < nppl:
                if w[j] > 0:
...
#        pdb.set_trace()
        ...
        #Add in complement, negative etc.  
