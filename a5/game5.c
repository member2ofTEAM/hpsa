...
int exact_pull(int *pos)
{
    ...
    {
        d = distance_squared(pos[0], pos[1], abs(moves[i * 2]), abs(moves[i * 2 + 1]));
        if (moves[i * 2] > 0)
            pull += d;
        else
            pull -= d;
...
int execute_move(int *move, int do_flag, int algo)
{
    ...
    else
    {
        ...
        //Restore its value
        board[move[0]][move[1]] = exact_pull(move);
    }
...
void get_player_move(int move_no, int player, int *move)
{
    //TODO: READ OVER THIS AGAIN
    if (player)
    {
        move[0] = moves[move_no * 2 + 2];
        move[1] = moves[move_no * 2 + 2 + 1];
    }
    else
    {
        move[0] = moves[move_no * 2];
        move[1] = moves[move_no * 2 + 1];
    }
}
