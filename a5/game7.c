#include <stdio.h>
#include "tinymt32.h"
#include <stdlib.h>
#include <math.h>
#include <sys/types.h>
#include <unistd.h>
#include <inttypes.h>
#include "tinymt32.c"
#include <time.h>
#include <assert.h>

void test_algorithm();
int do_move_random(int *move);
int do_move_algorithm(int *move);
int do_move_manual(int *move);
void print_board();
void decent_random(int *move);
void alpha_better(int *move);
int value(int alpha, int beta, int depth, int max);
int eval_fn();
int our_area();
void update_point(int *pos, int do_flag, int *move);
int exact_pull(int *pos);
int next_point(int *move, int which, int algo);
int get_random_int();


#define max(a,b) \
   ({ __typeof__ (a) _a = (a); \
          __typeof__ (b) _b = (b); \
               _a > _b ? _a : _b; })

#define min(a,b) \
   ({ __typeof__ (a) _a = (a); \
          __typeof__ (b) _b = (b); \
               _a < _b ? _a : _b; })


const int BOARD_SIZE = 1000;
const double INF = 10000000;
double board[1000][1000];
int NUM_MOVES_REMAINING = 30;
int MAX_NUMBER_OF_MOVES = 30;
int moves[60] = {-1};
int MAX_NUMBER_OF_POINTS = 1000000;
int next_to_set;
//Player 1 gets to start
int player_to_start = 0;
tinymt32_t state;

/*
 *
 * CHANGING THE BOARD
 *
 */

void init_board_from_moves()
{
    //TODO NEED TO FLIP NEXT_TO_SET AS MANY TIMES AS THERE ARE MOVES!
    int pos[2], i;
    for (i = 0; i < MAX_NUMBER_OF_POINTS; i++)
    {
        if (!next_point(pos, i, 0))
            break;
        board[pos[0]][pos[1]] = exact_pull(pos);
    }
}

double distance_squared(int x0, int y0, int x1, int y1)
{
    return (x0 - x1) * (x0 - x1) + (y0 - y1) * (y0 - y1);
}

void update_point(int *pos, int do_flag, int *move)
{
    int i = pos[0], j = pos[1];
    double d = 0.0;
    //Ignore pixels with stones set
    if (!((abs(board[i][j]) == INF) || (move[0] == i && move[1] == j)))
    {
        d = distance_squared(move[0], move[1], i, j);
        if (!do_flag)
            d = -d;
        board[i][j] += 1.0 / d;
    }
}

int exact_pull(int *pos)
{
    int i = 0;
    double d = 0.0, pull;
    for (i = 0; i < MAX_NUMBER_OF_MOVES - NUM_MOVES_REMAINING; i++)
    {
        d = distance_squared(pos[0], pos[1], moves[i * 2], moves[i * 2 + 1]);
        if (i % 2)
            pull += d;
        else
            pull -= d;
    }
    return pull;
}

int execute_move(int *move, int do_flag, int algo)
{
    int next[2], i;
    
    //Make sure the move is valid
    if (move[0] < 0 || move[0] > 999 || move[1] < 0 || move[1] > 999)
        return -1;

    if (do_flag)
    {
        //Make sure stone can be placed
        if (abs(board[move[0]][move[1]]) == INF)
            return -1;
        //Place the stone
        if (next_to_set)
            board[move[0]][move[1]] = INF;
        else
            board[move[0]][move[1]] = -INF;
    }
    else
    {
        //Make sure stone can be removed
        if (abs(board[move[0]][move[1]]) != INF)
            return -1;
        //Remove the stone
        board[move[0]][move[1]] = 0;
        //Restore its value
        board[move[0]][move[1]] = exact_pull(move);//very nice, but really necessary? we update the board afterwards...
    }                                              //just set it to 0

    //Update the board
    for(i = 0; i < MAX_NUMBER_OF_POINTS; i++)
    {
        if (!next_point(next, i, 0))
            break;
        //If we are not next_to_set subtract the pull
        update_point(next, next_to_set, move);
    }

    //Store the move (only if do_flag?) 
    if (do_flag)
    {
        i = MAX_NUMBER_OF_MOVES - NUM_MOVES_REMAINING;
        moves[i * 2]       = move[0];
        moves[(i * 2) + 1] = move[1];
    }
    else
    {
        i = MAX_NUMBER_OF_MOVES - NUM_MOVES_REMAINING - 1;
        moves[i * 2]       = -1;
        moves[(i * 2) + 1] = -1;
    }

    //Flip the player
    next_to_set = next_to_set > 0 ? 0 : 1;

    // Adjust number of moves remaining;
    if (do_flag)
        NUM_MOVES_REMAINING--;
    else
        NUM_MOVES_REMAINING++;

    return our_area();
}

//move is in the format [x, y] as a 2 dim array
//returns the area owned by us
int do_move(int *move)
{
    return execute_move(move, 1, 0);
}

//move is in the format [x, y] as a 2 dim array
int undo_move(int *move)
{
    return execute_move(move, 0, 0);
}

int do_move_coarse(int *move)
{
    return execute_move(move, 1, 1);
}

int undo_move_coarse(int *move)
{
    return execute_move(move, 0, 1);
}


int next_point(int *move, int which, int algo)
{
    int MAX_POINTS;
    if (algo == 1)
    {
        MAX_POINTS = 1000000;
        if (which > MAX_POINTS)
            return 0;
        move[0] = which / 1000;
        move[1] = which % 1000;
        return 1;
    }
    if (algo == 2)
    {
        MAX_POINTS = 400;
        if (which >= MAX_POINTS)
            return 0;
        move[0] = (which / 20) * 50;
        move[1] = (which % 20) * 50;
        return 1;
    }
    if (algo == 3)
    {
        //DO SOMETHING WITH GAME BOARD
        //MAKE ARRAY OF POINTS
        MAX_POINTS = 40;
        if (which > MAX_POINTS)
            return 0;
        //SET MOVE TO which POINT
        return 1;
     }
     return 0;
}


/*
 *
 *
 *
 */

/*
 *
 * PERFORMING TEST / STATS
 *
 */


//we could probably just go over the sample points,too
int our_area()
{
    int i, j, area = 0;
    for (i = 0; i < BOARD_SIZE; i++)
    {
        for (j = 0; j < BOARD_SIZE; j++)
        {
           if (board[i][j] > 0)
               area++;
        }
    }
    return area;
}

void print_board()
{
    int i, j;
    for (i = 0; i < BOARD_SIZE; i++)
    {
        for (j = 0; j < BOARD_SIZE; j++)
        {
            printf("%17.16lf ", board[i][j]);
        }
        printf("\n");
    }
    printf("\n");
}

void save_stats(int score)
{
    FILE *stats;
    int i;
    stats = fopen("output.csv", "a+");
    assert(stats);
    fprintf(stats, "%d ", player_to_start);
    fprintf(stats, "%d", score);
    for(i = 0; i < MAX_NUMBER_OF_MOVES; i++)
    {
       fprintf(stats, ", %d %d", moves[i * 2], moves[(i * 2) + 1]);
    }
    fprintf(stats, "\n");
    
}

void test_algorithm()
{
    int score, move[2];
    while(NUM_MOVES_REMAINING)
    {
        score = do_move_manual(move);
        score = do_move_algorithm(move);
        printf("%d %d\n", move[0], move[1]);

    }
    printf("%d", our_area());
    //print_board();
    if(score <= 500000)
       save_stats(score);
}

/*
 *
 *
 *
 */

int main(int argc, char *argv[])
{
//    NUM_MOVES_REMAINING = atoi(argv[1]);
//    init_board();
    int i;
    uint32_t seed = getpid();
    tinymt32_init(&state, seed);
    next_to_set = 1;
    for(i = 0; i < 1; i++)
       test_algorithm();
    return 0;
}

/*
 *
 * MAKING MOVES
 *
 */

int do_move_manual(int *move)
{
    int  score = -1;
    while(score < 0)
    {
        scanf("%d,%d", &move[0], &move[1]);
        printf("move received\n");
        score = do_move(move);
    }
    return score;
}

/* maybe we should pass an int value for the algorithm
 * we want to use
 * like: do_move_algorithm(move, 1)
 * which then calls do_move(move,1)
 * -> and then we change algo to 1 in do_move
*/ 
int do_move_algorithm(int *move)
{
    int score;
    alpha_better(move);
    score = do_move(move);
    return(score);
}

int do_move_random(int *move)
{
    move[0] = get_random_int();
    move[1] = get_random_int();
    int score;
    while((score = do_move(move)) < 0)
    {
        move[0] = get_random_int();
        move[1] = get_random_int();
    }
    return score;
}

/*
 *
 *
 *
 */

/*
 *
 * MOVE ALGORITHMS
 *
 */

/* RANDOM ALGORITHM */
 
//Between 0 and 999 inclusive
int get_random_int()
{
    //abs?
    return ((int)((unsigned int) tinymt32_generate_uint32(&state) % 1000));
}
// Randomly where we don't own 
void decent_random(int *move)
{
    move[0] = get_random_int();
    move[1] = get_random_int();
    while(do_move(move) < 0 || board[move[0]][move[1]] > 0)
    {
        move[0] = get_random_int();
        move[1] = get_random_int();
    }
    undo_move(move);
}

/* */

/* ALPHA BETA */

int eval_fn()
{
    return our_area();
}

int value(int alpha, int beta, int depth, int max)
{
    int v = -INF, i, move[2];
    if (depth > min(0, NUM_MOVES_REMAINING)){
        return eval_fn();
    }
    
    for (i = 0; i < MAX_NUMBER_OF_POINTS; i++)
    {
        if(!next_point(move, i, 2))
            break;
        if(do_move(move) > 0)
        {
            if (max)
            {
                v = max(v, value(alpha, beta, depth + 1, 0));
                if (v >= beta)
                {
                    undo_move(move);
                    return v;
                }
                alpha = max(alpha, v);
            }
            else
            {
                v = min(v, value(alpha, beta, depth + 1, 1));
                if (v <= alpha)
                {
                    undo_move(move);
                    return v;
                }
                beta = min(beta, v);
            }
            undo_move(move);
        }
    }
    return v;
}
                                 
/* Recursively realizes feasible sequences of moves and calls
 * the evaluation function
 */
void alpha_better(int *move)
{
    int best_v = 2 * INF, v = INF, i;
    int best_move[2];
    int max = next_to_set;

    if(max)
        best_v = -2 * INF;
     
    for (i = 0; i < MAX_NUMBER_OF_POINTS; i = i + 1)
    {
        if(!next_point(move, i, 2))
            break;
        if (do_move(move) > 0)
        {
            if (max)
            {
                v = value(-1 * INF, INF, 1, 0);
                if (v > best_v)
                {
                    best_v = v;
                    best_move[0] = move[0];
                    best_move[1] = move[1];
                }
            }
            else
            {
                v = value(-1 * INF, INF, 1, 1);
                if (v < best_v)
                {
                    best_v = v;
                    best_move[0] = move[0]; 
                    best_move[1] = move[1];
                }
            }
            undo_move(move);
        }
    }
    move[0] = best_move[0];
    move[1] = best_move[1];
} 

