...
f = open("prey_moves_diag", "wb")
#input_text = f.read().split('\n')
#input_text = 0
#offset = input_text[-3]
#prey_queue_input = eval(input_text[-2])
...
#    canvas.create_oval(x[t], y[t], x[t]+ball_diameter, y[t]+ball_diameter, fill="blue", tag='blueball')
    ...
    if tick % 2:
        ...
        if x_prey >= x_max or x_prey <= x_min or x_prey in wall_vertical.values():
            prey_move = (prey_move[0]*-1,prey_move[1])
            prey_queue = []
            prey_queue.append(prey_move)
            prey_queue = prey_queue*steps       
    
        if y_prey >= y_max or y_prey <= y_min or y_prey in wall_horizontal.values():
