...
while(1):
    ...
    for i in range(no_walls_build):
        ...
        #print walls
    wrm_flag = (len(old_walls) > len(walls))
    ...
    if not tick%20 and tick >=100 and prey_queue and not semaphore:
        calculate_heat(predict_steps,vx,vy,h_x,h_y,heatmap)
        del prey_queue[:]
#Only do this once!
#Change the strategy later on!
#Force these steps and make sure they happen, otherwise nonetype error!

    if wrm_flag and not semaphore:
        semaphore = 22;
#        pdb.set_trace()
        prey_queue = [(0, 1)] + 10 * [(1, 0)] + 10 * [(-1, 0)] + [(0, -1)] + prey_queue
